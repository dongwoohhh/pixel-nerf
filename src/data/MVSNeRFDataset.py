...
class MVSDatasetDTU(torch.utils.data.Dataset):
    def __init__(self, path, stage, n_views=3, levels=1, img_wh=None, downSample=1.0, max_len=-1):
        """
    ...
    def build_metas(self):
        self.metas = []
        with open(f'conf/lists/dtu_{self.split}_all.txt') as f:
        ...
        for scan in self.scans:
            with open(f'conf/dtu_pairs.txt') as f:
    ...
    def __getitem__(self, idx):
        ...
        focal = np.array([intrinsics[0, 0, 0], intrinsics[0, 1, 1]] , dtype=np.float32)
        c = np.array([intrinsics[0, 0, 2], intrinsics[0, 1, 2]] , dtype=np.float32)

        sample = {
            "images": imgs,
            "poses": c2ws.astype(np.float32),
            "focal": focal,
            "c": c,
            #"points": None,
            #"pcd_mask": None,
